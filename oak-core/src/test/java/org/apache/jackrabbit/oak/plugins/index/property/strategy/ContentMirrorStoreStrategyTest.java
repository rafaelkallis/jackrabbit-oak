...
import org.apache.jackrabbit.oak.plugins.document.WAPI;
...
public class ContentMirrorStoreStrategyTest {
    ...
    private static void checkPath(NodeBuilder node, String key, String path, boolean checkMatch) {
        path = PathUtils.concat(key, path);
        NodeBuilder check = node;
        for (String p : PathUtils.elements(path)) {
            Assert.assertTrue("Missing child node " + p + " on path " + path, check.hasChildNode(p));
            check = check.child(p);
        }
        if (checkMatch) {
    ...
    public void testUnique() throws CommitFailedException {
        ...
        Assert.assertTrue("ContentMirrorStoreStrategy should guarantee uniqueness on insert",
    ...
    public void testIndexCountersUsageWithoutPathRestriction() {
        ...
        Assert.assertEquals("Approximate count not used for is-not-null query", approxNodeCount,
                store.count(root, indexMeta.getNodeState(), null, maxTraversal));

        // prop=value query without entryCount
        key.setProperty(approxPropName, approxKeyCount, Type.LONG);
        Assert.assertEquals("Approximate count not used for key=value query", approxKeyCount,
                store.count(root, indexMeta.getNodeState(), KEY, maxTraversal));

        // is-not-null query with entryCount
        indexMeta.setProperty(ENTRY_COUNT_PROPERTY_NAME, entryCount, Type.LONG);
        Assert.assertEquals("Entry count not used even when present for is-not-null query", entryCount,
                store.count(root, indexMeta.getNodeState(), null, maxTraversal));
        ...
        Assert.assertTrue("Rough key count not considered for key=value query",
                entryCount > store.count(root, indexMeta.getNodeState(), KEY, maxTraversal));
    ...
    public void testIndexCountersUsageWithPathRestriction() {
        ...
        // is-not-null query without entryCount
        index.setProperty(approxPropName, approxNodeCount, Type.LONG);
        assertInRange("Approximate count not used for is-not-null query", approxNodeCount,
                store.count(filter, root, indexMeta.getNodeState(), null, maxTraversal));

        // prop=value query without entryCount
        key.setProperty(approxPropName, approxKeyCount, Type.LONG);
        assertInRange("Approximate count not used for key=value query", approxKeyCount,
                store.count(filter, root, indexMeta.getNodeState(), KEY, maxTraversal));

        // is-not-null query with entryCount
        indexMeta.setProperty(ENTRY_COUNT_PROPERTY_NAME, entryCount, Type.LONG);
        assertInRange("Entry count not used even when present for is-not-null query", entryCount,
                filteredNodeFactor * store.count(filter, root, indexMeta.getNodeState(), null, maxTraversal));

        // prop=value query with entryCount but without keyCount
        Assert.assertTrue("Rough key count not considered for key=value query", entryCount > filteredNodeFactor
                * store.count(filter, root, indexMeta.getNodeState(), KEY, maxTraversal));

        // prop=value query with entryCount and keyCount
        indexMeta.setProperty(KEY_COUNT_PROPERTY_NAME, keyCount, Type.LONG);
        Assert.assertTrue("Key count not considered for key=value query", entryCount > filteredNodeFactor
                * store.count(filter, root, indexMeta.getNodeState(), KEY, maxTraversal));
    }
    ...
    private static void assertInRange(String msg, double expected, double actual) {
        ...
        Assert.assertTrue(msg + "; expected about " + expected + ", got " + actual, diff < expected * allowedError);
    }

    // @Test
    // public void testWAPI() throws CommitFailedException {
    //     IndexStoreStrategy store = new ContentMirrorStoreStrategy();

    //     MongoClient mongoClient = new MongoClient();
    //     DocumentNodeStore nodeStore = new DocumentMK.Builder().setMongoDB(mongoClient.getDB("oak-test-wapi"))
    //             .setClusterId(1).getNodeStore();
    //     WAPI.enableWorkloadAwareness(nodeStore, 2, 24 * 60 * 60 * 1000);

    //     NodeBuilder builder = nodeStore.getRoot().builder();
    //     Supplier<NodeBuilder> index = () -> builder;

    //     for (String path : asList("/")) {
    //         store.update(index, path, null, null, EMPTY, KEY);
    //     }
